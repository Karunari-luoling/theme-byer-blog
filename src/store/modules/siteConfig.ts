...
export const useSiteConfigStore = defineStore("anheyu-site-config", {
  ...
  getters: {
    ...
    // 是否开启多人共创功能（允许普通用户发布文章）
    enableMultiAuthor: state => {
      const value =
        state.siteConfig?.["article.multi_author.enable"] ??
        state.siteConfig?.article?.multi_author?.enable;
      return value === true || value === "true";
    },
    // 普通用户发布的文章是否需要审核
    multiAuthorNeedReview: state => {
      const value =
        state.siteConfig?.["article.multi_author.need_review"] ??
        state.siteConfig?.article?.multi_author?.need_review;
      return value === true || value === "true";
    }
  },
