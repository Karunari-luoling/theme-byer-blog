...
router.beforeEach(async (to: ToRouteType, _from, next) => {
  ...
  if (storageLocal().getItem(multipleTabsKey) && userInfo) {
    ...
    // 4.1 权限检查（排除文章管理页面，因为需要先检查多人共创配置）
    if (
      to.meta?.roles &&
      !to.path.startsWith("/admin/post-management") &&
    ...
    // 4.1.1 多人共创配置检查：普通用户访问文章管理需要检查配置是否开启
    if (!isAdmin && to.path.startsWith("/admin/post-management")) {
      // 先检查角色权限
      ...
      if (!siteConfigStore.isLoaded) {
        console.log("[路由守卫] 配置未加载，开始加载配置...");
        await siteConfigStore.fetchSiteConfig();
      }

      // 检查多人共创配置是否开启
      const enableMultiAuthor = siteConfigStore.enableMultiAuthor;
      console.log(
        "[路由守卫] 多人共创配置检查",
        "enableMultiAuthor:",
        enableMultiAuthor,
      ...
      if (!enableMultiAuthor) {
        ElMessage.warning("多人共创功能未开启，普通用户暂不能访问文章管理");
        next({ path: "/error/403" });
        return;
      }
    }
    ...
    if (to.path === "/login") {
      if (isAdmin) {
        ...
        // 普通用户：如果开启了多人共创功能，跳转到文章管理页；否则跳转到首页
        const siteConfigStore = useSiteConfigStore();
        if (!siteConfigStore.isLoaded) {
          await siteConfigStore.fetchSiteConfig();
        }
        if (siteConfigStore.enableMultiAuthor) {
          // 多人共创开启，跳转到文章管理页
          next({ path: "/admin/post-management", replace: true });
        } else {
          // 多人共创未开启，跳转到首页
