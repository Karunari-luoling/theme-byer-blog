...
import type { MomentsSettingsInfo } from "../../../type";
...
defineOptions({
  name: "MomentsNotifySettings"
});

const model = defineModel<MomentsSettingsInfo>({ required: true });
...
const tip = `https://api.day.app/YOUR_KEY/{{.TITLE}}/{{.BODY}}?isArchive=1&sound=health&icon={{.SITE_URL}}/favicon.ico&group={{.SITE_NAME}}&url={{.MOMENTS_URL}}`;
...
<template>
  <div>
    ...
    <el-row :gutter="20">
      <el-col :span="12">
        <el-form-item label="抓取到新文章时通知站长">
          ...
          <el-form-item label="自定义请求体模板">
            <div class="json-input-container">
              <el-input
                ...
                placeholder='{"title":"#{TITLE}","content":"#{BODY}","site_name":"#{SITE_NAME}","link_name":"#{LINK_NAME}","post_title":"#{POST_TITLE}","post_url":"#{POST_URL}","moments_url":"#{MOMENTS_URL}","time":"#{TIME}"}'
...
User-Agent: AnheyuBlog-Webhook/1.0"
    ...
    <div class="template-hint">
      ...
      <ul>
        ...
        <li @click="handleCopy('{{.SITE_URL}}')">
          <code v-pre>{{.SITE_URL}}</code
          >: 网站地址
        </li>
        <li @click="handleCopy('{{.MOMENTS_URL}}')">
          <code v-pre>{{.MOMENTS_URL}}</code
          >: 朋友圈页面链接
        </li>
        ...
        <li @click="handleCopy('{{.POST_TITLE}}')">
          <code v-pre>{{.POST_TITLE}}</code
          >: 文章标题
        </li>
      ...
      <ul>
        ...
        <li @click="handleCopy('#{SITE_URL}')">
          <code>#{SITE_URL}</code>: 网站地址
        </li>
        <li @click="handleCopy('#{MOMENTS_URL}')">
          <code>#{MOMENTS_URL}</code>: 朋友圈页面链接
        </li>
        <li @click="handleCopy('#{LINK_NAME}')">
          <code>#{LINK_NAME}</code>: 友链名称
        </li>
        <li @click="handleCopy('#{POST_TITLE}')">
          <code>#{POST_TITLE}</code>: 文章标题
        </li>
        <li @click="handleCopy('#{POST_URL}')">
          <code>#{POST_URL}</code>: 文章链接
        </li>
        <li @click="handleCopy('#{TIME}')"><code>#{TIME}</code>: 抓取时间</li>
      </ul>
    </div>

    <el-divider content-position="left">邮件模板设置</el-divider>
    <el-form-item label="站长收到新文章通知的邮件主题">
      <el-input
        v-model="model.mailSubjectAdmin"
        placeholder="{{.SITE_NAME}} 朋友圈有新文章"
      />
    </el-form-item>
    <el-form-item label="站长收到新文章通知的邮件内容模板 (支持HTML)">
