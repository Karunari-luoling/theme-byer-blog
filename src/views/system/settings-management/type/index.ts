 ...
 * @LastEditTime: 2025-10-27 12:12:01
 * @LastEditors: 安知鱼
 */
...
export interface PostSettingsInfo {
  ...
  multiAuthor: {
    enable: boolean; // 是否开启多人共创功能（允许普通用户发布文章）
    needReview: boolean; // 普通用户发布的文章是否需要审核
  };
  reviewNotify: {
    enable: boolean; // 是否启用文章审核通知
    email: boolean; // 是否发送邮件通知
    push: boolean; // 是否发送即时通知
    pushChannel: string; // 推送平台 (bark/webhook)
    pushURL: string; // 推送URL
    webhookRequestBody: string; // Webhook 自定义请求体
    webhookHeaders: string; // Webhook 自定义请求头
    mailSubjectApproved: string; // 审核通过邮件主题
...
export interface SidebarPageSettingsInfo {
  ...
  weatherEnablePage: string;
  weatherQweatherKey: string;
  weatherQweatherAPIHost: string;
  weatherIPAPIKey: string;
  weatherLoading: string;
  weatherDefaultRectangle: boolean;
...
/**
 * @description: 即刻（说说）配置表单接口 - PRO 专属
 */
export interface EssaySettingsInfo {
  title: string;
  subTitle: string;
  tips: string;
  buttonText: string;
...
export interface MomentsSettingsInfo {
  enable: boolean;
  title: string;
  subTitle: string;
  tips: string;
  ...
  // 通知配置
  notifyAdmin: boolean; // 是否通知站长（抓取到新文章时）
  scMailNotify: boolean; // 同时通过邮件和IM通知
  pushooChannel: string; // 推送平台 (bark/webhook)
  pushooURL: string; // 推送URL
  webhookRequestBody: string | object; // Webhook 自定义请求体
  webhookHeaders: string; // Webhook 自定义请求头
  mailSubjectAdmin: string; // 站长收到新文章通知的邮件主题
  mailTemplateAdmin: string; // 站长收到新文章通知的邮件内容模板
}

/**
 * @description: 第三方登录配置表单接口 - PRO 专属
 */
export interface OAuthSettingsInfo {
  qq: {
    enable: boolean;
    appID: string;
    appKey: string;
...
/**
 * @description: SEO 搜索引擎提交配置表单接口
 */
export interface SeoSettingsInfo {
  autoSubmit: boolean;
  baidu: {
    enable: boolean;
    site: string;
