 ...
 * @LastEditTime: 2025-10-27 12:20:24
 * @LastEditors: 安知鱼
-->
...
const clockConfig = computed(() => {
  if (!props.config?.sidebar?.weather?.enable) return null;
  return {
    enable: props.config.sidebar.weather.enable,
    qweatherKey: props.config.sidebar.weather.qweather_key,
    qweatherAPIHost: props.config.sidebar.weather.qweather_api_host,
    ipAPIKey: props.config.sidebar.weather.ip_api_key,
    loading: props.config.sidebar.weather.loading,
    defaultRectangle: props.config.sidebar.weather.default_rectangle,
    rectangle: props.config.sidebar.weather.rectangle
  };
});
...
<template>
  <div class="sticky-container">
    ...
    <!-- 天气时钟 -->
    <CardClock v-if="clockConfig" :config="clockConfig" />
