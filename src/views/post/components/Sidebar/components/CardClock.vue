...
const props = defineProps({
  config: {
    type: Object as PropType<WeatherConfig>,
    required: true
  }
});
...
const updateTime = () => {
  ...
  currentTime.value = `${zeroPadding(cd.getHours(), 2)}:${zeroPadding(cd.getMinutes(), 2)}:${zeroPadding(cd.getSeconds(), 2)}`;
  currentDate.value = `${zeroPadding(cd.getFullYear(), 4)}-${zeroPadding(cd.getMonth() + 1, 2)}-${zeroPadding(cd.getDate(), 2)} ${week[cd.getDay()]}`;

  const hour = cd.getHours();
  currentPeriod.value = hour >= 12 ? " P M" : " A M";
};

// 根据经纬度获取城市名称
const fetchCityName = async (location: string): Promise<string> => {
  try {
    const response = await fetch(
      `https://geoapi.qweather.com/v2/city/lookup?location=${location}&key=${props.config.qweatherKey}`
    );
    ...
    console.error("城市名获取失败:", error);
    return "未知";
  }
};

// 获取天气信息
const fetchWeatherInfo = async (location: string): Promise<void> => {
  try {
    const response = await fetch(
      `https://${props.config.qweatherAPIHost}/v7/weather/now?location=${location}&key=${props.config.qweatherKey}`
    );
    const data = await response.json();

    if (data.code === "200") {
      weatherInfo.value = data;
      currentColor.value = getWeatherColor(data.now.icon);
...
const getIpInfo = async (): Promise<void> => {
  if (props.config.defaultRectangle) {
    // 使用固定坐标时，通过坐标反查城市名
    const location = props.config.rectangle;
    ...
    <div class="card-glass">
      <div class="card-background">
        <div v-if="isLoading" class="loading-container">
          <img :src="config.loading" alt="Loading" class="loading-img" />
        </div>
        <div v-else class="card-content">
          <div id="hexo_electric_clock">
            <div class="clock-row">
              <span class="card-clock-clockdate">{{ currentDate }}</span>
              <span v-if="weatherInfo" class="card-clock-weather">
                <i
                  :class="`qi-${weatherInfo.now.icon}-fill`"
                  :style="{ color: currentColor }"
                />
                {{ weatherInfo.now.text }}
                <span class="temp">{{ weatherInfo.now.temp }}</span> ℃
              </span>
            </div>
            <div class="clock-row">
              <span class="card-clock-time">{{ currentTime }}</span>
            </div>
            <div class="clock-row">
              <span v-if="weatherInfo" class="card-clock-windDir">
                <i
                  class="qi-wind"
                  :style="{
                    transform: `rotate(${weatherInfo.now.wind360}deg)`,
              ...
              <span class="card-clock-dackorlight">{{ currentPeriod }}</span>
            </div>
          </div>
        </div>
      </div>
...
<style lang="scss">
/* 全局字体定义 - 不使用 scoped，确保字体正确加载 */
@font-face {
  font-family: UnidreamLED;
  src: url(https://cdn.cbd.int/anzhiyu-blog@1.1.6/fonts/UnidreamLED.ttf);
  font-display: swap;
}
...
@font-face {
  ...
  src:
    url("https://cdn.cbd.int/hexo-butterfly-clock-anzhiyu/lib/fonts/qweather-icons.woff2")
      format("woff2"),
    url("https://cdn.cbd.int/hexo-butterfly-clock-anzhiyu/lib/fonts/qweather-icons.woff")
      format("woff"),
    url("https://cdn.cbd.int/hexo-butterfly-clock-anzhiyu/lib/fonts/qweather-icons.ttf")
      format("truetype");
}
...
.qi-freezing-rain-icing::before {
  content: "\f195";
}
.qi-ground-frost::before {
  content: "\f196";
}
.qi-dust-raising-winds::before {
  content: "\f197";
}
.qi-strong-surface-winds::before {
  content: "\f198";
}
.qi-hot-day::before {
  content: "\f199";
}
.qi-warm-night::before {
  content: "\f19a";
}
.qi-cold-day::before {
  content: "\f19b";
}
.qi-thunderstorm-and-lightning::before {
  content: "\f19c";
}
.qi-hailstorm::before {
  content: "\f19d";
}
.qi-sea-area-warning::before {
  content: "\f19e";
}
.qi-fishermen-warning::before {
  content: "\f19f";
}
.qi-low-humidity::before {
  content: "\f1a0";
}
.qi-accumulated-rain::before {
  content: "\f1a1";
}
.qi-hazardous-surf-warning::before {
  content: "\f1a2";
}
.qi-marine-wind-warning::before {
  content: "\f1a3";
}
.qi-road-weather-alert::before {
  content: "\f1a4";
}
.qi-thunder-rain::before {
  content: "\f1a5";
}
.qi-thunder-rain-and-fog::before {
  content: "\f1a6";
}
.qi-damaging-winds::before {
  content: "\f1a7";
}
.qi-veld-fire-conditions::before {
  content: "\f1a8";
}
.qi-weather-advisory::before {
  content: "\f1a9";
}
