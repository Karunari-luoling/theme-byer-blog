    ...
    AnHttp.axiosInstance.interceptors.response.use(
      ...
      async (error: AnHttpError) => {
        ...
        const isOAuthRequest =
          ...
          config.url?.includes("/auth/gitlab") ||
          config.url?.includes("/auth/gitee") ||
          config.url?.includes("/auth/callback");
        if (isOAuthRequest) {
          // 对于OAuth授权相关的错误，直接将原始错误抛出
          // 让业务组件的 .catch 去处理，以便控制按钮的 loading 状态
          console.warn(
            "检测到OAuth授权接口错误，将错误传递到业务层处理:",
            response?.data?.message || "未知错误"
          );
          // 注意：抛出后端的标准响应体，方便业务层直接读取 message/code
          // 例如 { code: 403, message: "您的账户已被封禁...", data: null }
