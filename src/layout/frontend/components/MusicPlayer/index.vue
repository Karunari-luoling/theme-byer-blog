 ...
 * @LastEditTime: 2025-09-29 16:42:45
 * @LastEditors: 安知鱼
-->
<template>
  <div
...
const handleMusicControlEvents = () => {
  ...
  // 监听 Essay 音乐播放器的播放事件，避免冲突
  const handleEssayMusicPlay = () => {
    if (audioPlayer.audioState.isPlaying) {
      console.log(" [音乐胶囊] 检测到 Essay 音乐播放，暂停当前音乐");
      audioPlayer.togglePlay();
    }
  };

  // 监听 AI 播客播放事件，避免冲突
  const handleAIPodcastPlay = () => {
    if (audioPlayer.audioState.isPlaying) {
      console.log(" [音乐胶囊] 检测到 AI 播客播放，暂停当前音乐");
      audioPlayer.togglePlay();
    }
  };
  ...
  window.addEventListener("ai-podcast-play", handleAIPodcastPlay);
  ...
  return () => {
    ...
    window.removeEventListener("ai-podcast-play", handleAIPodcastPlay);
...
#nav-music {
  ...
  left: 20px;
  z-index: 1009;
  // 默认离开 footer 时有延迟出现（让 AI 播客按钮先出现）
