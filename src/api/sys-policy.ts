 ...
 * @LastEditTime: 2025-09-29 09:38:30
 * @LastEditors: 安知鱼
 */
...
export const updatePolicy = (data: Partial<StoragePolicy>) => {
  return http.request<Result<StoragePolicy>>(
    "put",
    baseUrlApi(`pro/policies/${data.id}`),
    {
      data
    }
  );
...
export const deletePolicy = (id: number) => {
  return http.request<Result<null>>("delete", baseUrlApi(`pro/policies/${id}`));
};
