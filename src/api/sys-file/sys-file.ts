...
import {
  ...
  type VerifySharePasswordRequest,
  type VerifySharePasswordResponse,
...
/**
 * @description 创建分享链接
 * @param {CreateShareLinkRequest} params 分享参数
 * @returns {Promise<CreateShareLinkResponse>}
 */
export const createShareLinkApi = (
  params: CreateShareLinkRequest
): Promise<CreateShareLinkResponse> => {
  return http.post<CreateShareLinkResponse, CreateShareLinkRequest>(
    baseUrlApi("files/share/create"),
    params
  );
};
 ...
 * @param {string} [token] 访问令牌（如果需要密码验证）
 * @returns {Promise<GetShareInfoResponse>}
 */
export const getShareInfoApi = (
  shareId: string,
  sign?: string
): Promise<GetShareInfoResponse> => {
  const params = sign ? { sign } : {};
  return http.get<GetShareInfoResponse, any>(
    baseUrlApi(`files/share/${shareId}`),
    params
  );
};
...
/**
 * @description 验证分享密码
 * @param {VerifySharePasswordRequest} params 验证参数
 * @returns {Promise<VerifySharePasswordResponse>}
 */
export const verifySharePasswordApi = (
  params: VerifySharePasswordRequest
): Promise<VerifySharePasswordResponse> => {
  return http.post<VerifySharePasswordResponse, VerifySharePasswordRequest>(
    baseUrlApi("files/share/verify-password"),
    params
  );
};
 ...
 * @param {string} [sign] 访问签名（如果需要密码验证）
 * @returns {Promise<Blob>} 文件流
 */
export const downloadShareFileApi = async (
  shareId: string,
  fileId: string,
  sign?: string
): Promise<Blob> => {
  const url = sign
    ? baseUrlApi(
        `files/share/${shareId}/download/${fileId}?sign=${encodeURIComponent(sign)}`
      )
    : baseUrlApi(`files/share/${shareId}/download/${fileId}`);
  const response = await http.request<Blob>("get", url, {
    responseType: "blob"
  });
  return response;
};
...
/**
 * @description 批量下载分享的所有文件（打包为ZIP）
 * @param {string} shareId 分享 ID
 * @param {string} [sign] 访问签名（如果需要密码验证）
 * @returns {Promise<Blob>} ZIP文件
 */
export const downloadShareAllFilesApi = async (
  shareId: string,
  sign?: string
): Promise<Blob> => {
  const url = sign
    ? baseUrlApi(
        `files/share/${shareId}/download-all?sign=${encodeURIComponent(sign)}`
      )
    : baseUrlApi(`files/share/${shareId}/download-all`);
  const response = await http.request<Blob>("post", url, {
    responseType: "blob"
  });
  return response;
};
...
/**
 * @description 获取分享的文件列表（完全参考社区版 fetchFilesByPathApi 的设计）
 * @param {string} uri URI 路径（如 "anzhiyu://shareId/" 或 "anzhiyu://shareId/folderId"）
 * @param {string} [sign] 访问签名（如果需要密码验证）
 * @returns {Promise<GetShareFileListResponse>} 文件列表
 */
export const getShareFileListApi = async (
  uri: string,
  sign?: string
): Promise<GetShareFileListResponse> => {
  const params: any = { uri };
  if (sign) {
    params.sign = sign;
  }
  return http.get<GetShareFileListResponse, any>(
    baseUrlApi(`files/share/file`),
    params
  );
};

/**
 * @description 获取分享文件的预览URL列表
 * @param {string} shareId 分享 ID
 * @param {string} fileId 文件 ID
 * @param {string} [sign] 访问签名（如果需要密码验证）
 * @returns {Promise<FilePreviewUrlsResponse>}
 */
export const getShareFilePreviewUrlsApi = (
  shareId: string,
  fileId: string,
  sign?: string
): Promise<FilePreviewUrlsResponse> => {
  ...
  return http.request<FilePreviewUrlsResponse>(
    "get",
    baseUrlApi(`files/share/${shareId}/preview-urls`),
    {
      params
    }
  );
