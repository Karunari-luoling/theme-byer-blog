 ...
 * @LastEditTime: 2025-09-14 22:03:26
 * @LastEditors: 安知鱼
 */
...
export interface CreateShareLinkRequest {
  file_ids: string[]; // 要分享的文件/文件夹 ID 数组
  expiration_days?: number; // 过期天数（可选）
  payment_amount?: number; // 付费金额（可选）
  password?: string; // 访问密码（可选）
  show_readme?: boolean; // 是否显示 README（仅文件夹，可选）
  download_limit?: number; // 下载次数限制（可选）
  allowed_user_groups?: number[]; // 允许访问的用户组列表（可选，默认 [1,2,3]）
}
...
export interface ShareLinkData {
  ...
  expiration_time?: string; // 过期时间（ISO 8601 格式）
  download_limit?: number; // 下载次数限制
...
export interface ShareInfoData {
  ...
  download_limit?: number; // 下载次数限制
  payment_amount?: number; // 付费金额
  show_readme: boolean; // 是否显示 README
  readme_content?: string; // README 内容（如果有）
  requires_password: boolean; // 是否需要密码
  allowed_user_groups?: number[]; // 允许访问的用户组列表
  access_denied?: boolean; // 当前用户组无权访问
  requires_upgrade?: boolean; // 是否需要升级用户组（登录）
}

/**
 * 获取分享信息的响应体
 */
export type GetShareInfoResponse = BaseResponse<ShareInfoData>;

/**
 * 获取分享文件列表的响应数据（类似 FileListResponse）
 */
export interface ShareFileListData {
  files: FileItem[]; // 文件列表
  next_token?: string; // 分页游标（预留）
  path: string; // 当前路径（URI）
  folder_id?: string; // 当前文件夹ID
 ...
 * 获取分享文件列表的响应体
 */
export type GetShareFileListResponse = BaseResponse<ShareFileListData>;
 ...
 * 验证分享密码的响应体
 */
export type VerifySharePasswordResponse = BaseResponse<{
  token: string; // 访问令牌
}>;
