<template>
  <AnDialog
    ...
    hide-footer
    class="product-payment-dialog-wrapper"
  >
    <div class="product-payment-dialog">
      <!-- 第一步：填写信息 -->
          ...
          >
            <el-form-item label="" prop="email" :required="true">
              <el-input
                ...
                clearable
              />
              <div class="form-help">用于接收购买凭证和发货信息</div>
            </el-form-item>
          </el-form>
        </div>
      </div>
      ...
      <div v-if="paymentSuccess" class="payment-success">
        ...
        <p class="success-desc">
          {{ successMessage || "发货信息已发送至您的邮箱，请注意查收" }}
        </p>
        <div class="support-link">
          <span class="support-text">遇到问题？</span>
          <el-link type="primary" @click="handleOpenSupport">
...
import { Picture, CircleCheck, Service } from "@element-plus/icons-vue";
import AnDialog from "@/components/AnDialog/index.vue";
import { createProductOrderApi } from "@/api/product";
...
const emit = defineEmits<{
  ...
  success: [];
  openSupport: [orderNo: string, userEmail: string];
}>();
...
const handleOpenSupport = () => {
  emit("openSupport", orderNo.value, userForm.value.email);
};
